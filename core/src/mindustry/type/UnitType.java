...
public class UnitType extends UnlockableContent implements Senseable{
    ...
    public void drawShield(Unit unit){
        ...
        Fill.light(unit.x, unit.y, Lines.circleVertices(radius), radius,
            Color.clear,
            Tmp.c2.set(unit.type.shieldColor(unit)).lerp(Color.white, Mathf.clamp(unit.hitTime() / 2f)).a(0.7f * unit.shieldAlpha() * currentAlpha)
        );
    }
    ...
    public void drawItems(Unit unit){
        ...
        if(unit.item() != null && unit.itemTime > 0.01f){
            ...
            if((drawAllItems || unit.isLocal()) && !renderer.pixelate){
                float z = Draw.z();
    ...
    public void drawTrail(Unit unit){
        ...
        Trail trail = unit.trail; // FINISHME: Apply alpha
    ...
    //TODO
    public void drawCrawl(Crawlc crawl){ // FINISHME: Apply alpha
