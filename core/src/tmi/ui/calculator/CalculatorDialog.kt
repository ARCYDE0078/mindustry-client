...
class CalculatorDialog: BaseDialog("") {
    ...
    addMenu(
      ...
      MenuTab(
        ...
        if (page.fi != null) {
          ...
          tmi.util.showFileChooser(false, page.title, "shd") { file ->
            if (save(page, file)) {
              page.fi = file
          ...
          viewPages.forEach {
            if (it.shouldSave()) {
              if (it.fi != null) {
                ...
                tmi.util.showFileChooser(false, it.title, "shd") { file ->
                  it.view.save(file)
                  deletePage(it)
                }
              }
  ...
  private fun buildTop() {
    ...
    //arc форка: Seq.groupBy -> ObjectMap перекрывает Kotlin-овский, нужен Map -> через asIterable()
    val tabs = topMenuTabSet.asIterable().groupBy { it.tabName }
  ...
  private fun buildFoldedMenu(pagesTable: Table) {
    ...
    pagesTable.button(
      ...
      if (currPage.fi != null) {
        ...
        tmi.util.showFileChooser(false, currPage.title, "shd") { file ->
              ...
              showChoiceIcons(
                ...
                Core.bundle["misc.save"] to Icon.save to Runnable {
                  if (page.fi != null) {
                    ...
                    tmi.util.showFileChooser(false, "shd") { file ->
                      save(page, file)
                      deletePage(page)
                    }
                  }
  ...
  private fun openFile(){
    tmi.util.showFileChooser(true, "shd") { file ->
